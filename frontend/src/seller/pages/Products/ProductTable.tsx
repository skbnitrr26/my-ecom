...
export default function ProductTable() {
  ...
  // 2. Create the handler function with confirmation
  const handleDeleteProduct = (id: number | undefined) => () => {
    if (id && window.confirm("Are you sure you want to delete this product? This action cannot be undone.")) {
              ...
              <StyledTableRow key={item.id}>
                <StyledTableCell component="th" scope="row">
                  <div className='flex gap-1 flex-wrap'>
                    {item.images.map((image, index) => <img key={index} className='w-12 rounded-md' src={image} alt="" />)}
                  </div>
                </StyledTableCell>
                ...
                {/* 3. Add the table cell with the delete button */}
                <StyledTableCell align="right">
                  <IconButton onClick={handleDeleteProduct(item.id)} sx={{ color: 'red' }}>
                    <DeleteIcon />
                  </IconButton>
                </StyledTableCell>
              </StyledTableRow>
